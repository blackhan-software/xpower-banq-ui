...
export const Position = {
    ...
    lockedTotalShare(position: Position, locked?: bigint): number {
        const lhs = Position.lockedTotal(position, locked);
        const rhs = Position.amount(position);
        if (rhs > 0) {
            return lhs / rhs;
        }
        return 0;
    },
    lockedShare(position: Position, locked?: bigint): number {
        const lhs = Position.locked(position, locked);
        const rhs = Position.amount(position);
        if (rhs > 0) {
            return lhs / rhs;
        }
