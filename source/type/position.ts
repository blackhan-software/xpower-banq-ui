...
export const Position = {
    ...
    checksum: (positions: Position[]): Amount => Number(
        positions.reduce((acc, p, i) => (acc + (1n + p.amount) * (1n + BigInt(i))) % 0xffffffffn, 0n)
    ),
    eq: (lhs_positions?: Position[], rhs_positions?: Position[]): boolean =>
        Position.checksum(lhs_positions ?? []) === Position.checksum(rhs_positions ?? []),
}
