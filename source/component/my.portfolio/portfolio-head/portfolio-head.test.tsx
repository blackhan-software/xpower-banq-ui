...
describe("PortfolioHead", () => {
    ...
    it("should show 'Good' title when ratio > 1.5", () => {
        const health = { supply: 200n, borrow: 100n };
        render(<PortfolioHead health={health as any} apy={null} mode={"supply" as any} />);
        const el = document.querySelector("[title*='Good']");
        expect(el).not.toBeNull();
    });
    it("should show 'Risk' title when 1.0 < ratio <= 1.5", () => {
        const health = { supply: 120n, borrow: 100n };
        render(<PortfolioHead health={health as any} apy={null} mode={"supply" as any} />);
        const el = document.querySelector("[title*='Risk']");
        expect(el).not.toBeNull();
    });
