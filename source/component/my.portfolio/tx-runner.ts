...
const call_error = async (
    ...
    if (e.data) {
        ...
        if (error?.name === "Locked") try {
            ...
            return `execeeded *locked* balance of ${kmg}; try to reduce the amount.`;
        } catch (_) {
            return `execeeded *locked* balance; try to reduce the amount.`;
        }
    }
    if (e.data) {
        ...
        if (error?.name === "ERC4626ExceededMaxRedeem") try {
            ...
            return `execeeded max. redeem of ${kmg}; try to reduce the amount.`;
        } catch (_) {
            return `execeeded max. redeem; try to reduce the amount.`;
        }
        if (error?.name === "ERC20InsufficientBalance") try {
            const assets = await v.convertToAssets(error.args[2]);
            const kmg = KMG_FORMAT(3)(Position.amount(p, assets));
            return `execeeded balance of ${kmg}; try to reduce the amount.`;
        } catch (_) {
            return `execeeded balance; try to reduce the amount.`;
        }
    }
    return fallback;
}
