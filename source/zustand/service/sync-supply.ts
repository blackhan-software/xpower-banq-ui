...
/**
 * @return A zustand store w/a sync-supply service.
 */
export function syncPortfolioSupply(
    ...
    store.subscribe(async (
        ...
        if (prev.actions.includes("portfolio_supply") &&
            next.actions.includes("portfolio_supply") === false
        ) {
            return; // avoid infinite loop
        }
        if (next.actions.includes("portfolio_supply")) {
            next.reset_actions("portfolio_supply");
            return; // avoid infinite loop
        }
        ...
        if ((next.wallet_account !== null && next.pool !== 0n) &&
            ...
            const token_contracts = token_addresses.map((ta) =>
                [pool_contract.supplyOf(ta), ta] as const
            ).map(([pa, ta]) =>
    ...
    // update portfolio supply-positions
    function update(
        ...
        return async (
            ...
            const { set_portfolio_supply } = store.getState();
            const { portfolio_supply } = store.getState();
            const { set_pool_supply } = store.getState();
            const { pool_supply } = store.getState();
            ...
            const old_positions = portfolio_supply?.get(pool_account) ?? [];
            ...
            /// portfolio-supply
            {
                const new_map = new Map(portfolio_supply);
                new_map.set(pool_account, new_positions);
                set_portfolio_supply(new_map);
            }
            /// pool-supply
            {
...
export default syncPortfolioSupply;
