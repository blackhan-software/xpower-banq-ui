...
export function syncHealthBy(
    ...
    store.subscribe(withActionGuard("portfolio_health", async (
        ...
        if ((next.wallet_account !== null && next.pool !== 0n) &&
            ...
            const token_contracts = await Promise.all(
                token_addresses.map(async (ta) =>
                    new PositionContract(await pool_contract[position_of](ta), runner)
                )
            );
